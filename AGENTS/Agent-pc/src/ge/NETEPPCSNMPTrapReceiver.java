 ...
 * 
 */
public class NETEPPCSNMPTrapReceiver implements CommandResponder {
	...
	public NETEPPCSNMPTrapReceiver() {
	}

        public static void startNETEPPCSNMPMonitoring() {
             System.out.println(".net monitoring starting ");
             try {
                NETEPPCSNMPTrapReceiver GEtrap = new NETEPPCSNMPTrapReceiver();
      ...
      **/         
              ...
              // timer.schedule(new MyTimerTask(), 0, 30000); // every 60 seconds
                ...
                Logger.getLogger(NETEPPCSNMPTrapReceiver.class.getName()).log(Level.SEVERE, null, ex);
            }
	...
	public void processPdu(CommandResponderEvent event) {
		...
		if ((n % 100 == 1)) {
			System.out.println("Processed "
                ...
                fileManager.writeToFile("system event is "+event.toString());
				...
				msg.append(var.toString()).append(";");
                               ...
                               // switch(oidDetail[0].trim()){
                                 switch(oidget){     
                                        ...
                                        String[] parts = oidDetail[1].trim().split("\\:");
                                        int seviarity =4;
                                        if(parts[0].equals("INFORMATION")){
                                            seviarity =3;
                                        }else if(parts[0].equals("WARNING")){
                                            seviarity =2;
                                        }else if(parts[0].equals("SEVERE")){
                                            seviarity =1;
                                        }
                                                   
                                                      alarm.setAlarm(oidDetail[1].trim());
                                                      alarm.setSeviarity(seviarity);
                                                      alarm.setAlarmId(2);
